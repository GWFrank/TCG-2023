...
def moveable_pieces(
    ...
    on_board = [piece1_on_board, piece2_on_board, piece3_on_board,
                piece4_on_board, piece5_on_board, piece6_on_board]
...
def determinacy(
    ...
    for dice in range(1, PIECE_NUM + 1):
        movables = moveable_pieces(dice, piece1_on_board, piece2_on_board,
                                   piece3_on_board, piece4_on_board, piece5_on_board, piece6_on_board)
...
print(
    f"constexpr ewn::move_t move_gen_single[{2*PIECE_NUM+1}][{BOARD_SIZE*BOARD_SIZE}][3] = ")
print("{")
...
print(
    f"constexpr int move_gen_single_count[{2*PIECE_NUM+1}][{BOARD_SIZE*BOARD_SIZE}] = ")
print("{")
print("{},")  # Skip the first row
for i in range(1, 2*PIECE_NUM+1):
    print("{", end="")
...
for i in range(1, PIECE_NUM+1):
    ...
    for j in range(2**PIECE_NUM):
        movables = moveable_pieces(
            i, j & 1, j & 2, j & 4, j & 8, j & 16, j & 32)
        print("{", end="")
...
for i in range(1, PIECE_NUM+1):
    print("{", end="")
    for j in range(2**PIECE_NUM):
        print(len(moveable_pieces(i, j & 1, j & 2,
              j & 4, j & 8, j & 16, j & 32)), end="")
        if j != 2**PIECE_NUM - 1:
            print(", ", end="")
    print("},")
print("};")
...
# Precompute random bits for zobrist hashing
random.seed(314159)

print("namespace zobrist {")
print(
    f"constexpr ewn::hash_t piece_pos_key[{2*PIECE_NUM+1}][{BOARD_SIZE*BOARD_SIZE}] = ")
print("{")
print("{},")  # Skip the first row
for i in range(1, 2*PIECE_NUM+1):
    print("{", end="")
    for j in range(BOARD_SIZE*BOARD_SIZE):
        print(hex(random.getrandbits(64)), end="")
        if j != BOARD_SIZE*BOARD_SIZE - 1:
            print(", ", end="")
    print("},")
print("};")
