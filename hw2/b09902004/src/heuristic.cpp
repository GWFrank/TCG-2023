...
int State::greedy() {
    ...
    for (int player = 0; player < 2; player++) {
        for (int i = 0; i < MAX_CUBES; i++) {
            int dist = shortest_distance(pos_[i + offset], player);
            if (min_dist[player] > dist) min_dist[player] = dist;
        }
        offset += MAX_CUBES;
    }
    if (next_ == BLUE) return min_dist[RED] - min_dist[BLUE];
    return min_dist[BLUE] - min_dist[RED];
}

int search(State &game, int alpha, int beta, int depth) {
    ...
    if (game.is_over()) {
        if (game.num_cubes_[RED] == 0) return game.next_ == BLUE ? WIN : LOSE;
        if (game.num_cubes_[BLUE] == 0) return game.next_ == RED ? WIN : LOSE;
        if (game.board_[0] >= MAX_CUBES) return game.next_ == BLUE ? WIN : LOSE;
        // if (is_red_cube(game.board[ROW * COL - 1]))
        return game.next_ == RED ? WIN : LOSE;
    }
    if (depth == 0) {
        return game.greedy();
    }
...
int search_and_get_move(State &game, int depth) {
    int move_arr[MAX_MOVES];
    int num_moves;
    int best_index = 0;
    int val = LOSE;
...
int get_random_move(State &game) {
    int move_arr[MAX_MOVES];
